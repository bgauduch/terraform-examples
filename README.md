...
| [`terraform-secrets-out-of-state`](examples/terraform-secrets-out-of-state/) | `lab` | `aws` `secrets` `ephemeral` `write-only` `rotation` `v1.11` | Keep a secret out of the state, one mechanism at a time: `sensitive`, ephemeral variables, ephemeral blocks, write-only arguments, then rotation. |
